...
int prepare_video_encoder(StreamingContext* sc,
  ...
  // Set the motion estimation search method and range
  av_opt_set(sc->video_avcc->priv_data, "me_method", "umh",
             AV_OPT_SEARCH_CHILDREN);
  av_opt_set(sc->video_avcc->priv_data, "me_range", "16",
             AV_OPT_SEARCH_CHILDREN);
  ...
  sc->video_avcc->bit_rate = 8 * 1000 * 1000;
  sc->video_avcc->rc_buffer_size = 16 * 1000 * 1000;
  sc->video_avcc->rc_max_rate = 8.5 * 1000 * 1000;
  sc->video_avcc->rc_min_rate = 8.0 * 1000 * 1000;

  sc->video_avcc->time_base = av_inv_q(input_framerate);
  sc->video_avs->time_base.den = sc->video_avcc->time_base.den;
  sc->video_avs->time_base.num = sc->video_avcc->time_base.num;
...
int encode_video(StreamingContext* decoder,
  ...
  while (response >= 0) {
    ...
    if (response == AVERROR(EAGAIN) || response == AVERROR_EOF) {
      ...
      logging("Error while receiving packet from encoder: %s",
              av_err2str(response));
      return -1;
    }
    ...
    if (response != 0) {
      logging("Error %d while receiving packet from decoder: %s", response,
              av_err2str(response));
      return -1;
    }
    ++count;
...
int transcode_audio(StreamingContext* decoder,
  ...
  while (response >= 0) {
    ...
    if (response == AVERROR(EAGAIN) || response == AVERROR_EOF) {
      ...
      logging("Error while receiving frame from decoder: %s",
              av_err2str(response));
      return response;
    }
...
int main(int argc, char* argv[]) {
  ...
  if (!sp.copy_video) {
    AVRational input_framerate =
        av_guess_frame_rate(decoder->avfc, decoder->video_avs, NULL);
    prepare_video_encoder(encoder, decoder->video_avcc, input_framerate, sp);
  } else {
    if (prepare_copy(encoder->avfc, &encoder->video_avs,
  ...
  if (!sp.copy_audio) {
    ...
    if (prepare_copy(encoder->avfc, &encoder->audio_avs,
  ...
  while (av_read_frame(decoder->avfc, input_packet) >= 0) {
    if (decoder->avfc->streams[input_packet->stream_index]
            ->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
      if (!sp.copy_video) {
        // TODO: refactor to be generic for audio and video (receiving a
        // function pointer to the differences)
    ...
    } else if (decoder->avfc->streams[input_packet->stream_index]
